...
func registerFlags(cfg *Config, proxies, proxyErrorsRetry *string) {
	...
	flag.DurationVar(&cfg.ProxyRefreshInterval, "proxy-refresh-interval", 0, "interval to refresh proxy list from a dedicated proxy list URL (0 to disable; disabled by default as Scrapoxy uses single gateway)")
	...
	flag.Float64Var(&cfg.ProxyValidationRate, "proxy-validation-rate", 0, "fraction of attempts to validate proxy connectivity (0-1; disabled by default as Scrapoxy manages upstream health)")
